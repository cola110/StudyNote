...
namespace ConsoleApp._WebClient
{
    ...
    public class AppTestDown
    {
        ...
        {
            AppTestDown a = new AppTestDown();
            HttpWebClientSelf x = new HttpWebClientSelf();


            a._K = 10;

            //订阅 DataReceive　事件 
            x.DataReceive += new HttpWebClientSelf.DataReceiveEventHandler(a.x_DataReceive);
            //订阅 ExceptionOccurrs　事件 
            x.ExceptionOccurrs += new HttpWebClientSelf.ExceptionEventHandler(a.x_ExceptionOccurrs);

            x.ThreadProcessEnd += new HttpWebClientSelf.ThreadProcessEventHandler(a.x_ThreadProcessEnd);
        ...
        private void x_DataReceive(HttpWebClientSelf Sender, DownLoadEventArgs e)
        {
            if (!this.b)
            {
        ...
        private void x_ExceptionOccurrs(HttpWebClientSelf Sender, ExceptionEventArgs e)
        {
        ...
        private void x_ThreadProcessEnd(HttpWebClientSelf Sender, ThreadProcessEventArgs e)
        {
    ...
    public class DownLoadState
    {
        ...
        public HttpWebClientSelf httpWebClient
        {
            get
            {
                return this._hwc;
        ...
        private HttpWebClientSelf _hwc;
    ...
    /// 支持断点续传多线程下载的类 
    /// </summary> 
    public class HttpWebClientSelf
    {
        private static object _SyncLockObject = new object();

        public delegate void DataReceiveEventHandler(HttpWebClientSelf Sender, DownLoadEventArgs e);

        public event DataReceiveEventHandler DataReceive; //接收字节数据事件

        public delegate void ExceptionEventHandler(HttpWebClientSelf Sender, ExceptionEventArgs e);

        public event ExceptionEventHandler ExceptionOccurrs; //发生异常事件

        public delegate void ThreadProcessEventHandler(HttpWebClientSelf Sender, ThreadProcessEventArgs e);
